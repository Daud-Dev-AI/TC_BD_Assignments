...
WHERE product_id = 101 AND stock_qty >= 2;

-- 3) Insert sale record
INSERT INTO Sales (sale_id, product_id, quantity_sold, sale_date, total_price)
VALUES (6, 101, 2, CURDATE(), 2 * (SELECT unit_price FROM Products WHERE product_id = 101));

-- If the UPDATE affected 0 rows (not enough stock), ROLLBACK; else COMMIT.
COMMIT;

-- Q8 List product names and their sales count
SELECT
p.product_name,
...
-- Q9 Sales where total_price > avg(total_price)
SELECT *
FROM Sales
WHERE total_price > (SELECT AVG(total_price) FROM Sales);

-- Q10 Performance implications of indexing sales date
-- Pros: Speeds up WHERE sale_date = ..., range filters (BETWEEN), and date-based grouping/ordering.
-- Cons: Slower inserts/updates (index must be maintained) and extra storage.
-- In your exercises, you frequently query by date/month, so indexing sale_date is commonly worth it.

-- Q11 Add CHECK constraint quantity_sold > 0
ALTER TABLE Sales
ADD CONSTRAINT chk_quantity_sold_positive
CHECK (quantity_sold > 0);

-- Q12 Make a view product_sales_info that holds product details + total sales per product
...
-- Q13 Create a stored procedure Update_Unit_Price(product_id, new_price)
DELIMITER $$
CREATE PROCEDURE Update_Unit_Price(IN in_product_id INT, IN in_new_price DECIMAL(10,2))
BEGIN
  UPDATE Products
  SET unit_price = in_new_price
  WHERE product_id = in_product_id;
END $$
DELIMITER ;
CALL Update_Unit_Price(101, 550.00);

-- Q14 Transaction: Insert new product and insert sales record
START TRANSACTION;

INSERT INTO Products (product_id, product_name, category, unit_price)
VALUES (106, 'Monitor', 'Electronics', 200.00);

INSERT INTO Sales (sale_id, product_id, quantity_sold, sale_date, total_price)
VALUES (6, 106, 2, '2024-01-04', 400.00);

COMMIT;

-- Q15 Total revenue per category for the year 2024
SELECT
p.category,
SUM(s.total_price) AS total_revenue_2024
FROM Sales s
JOIN Products p ON p.product_id = s.product_id
